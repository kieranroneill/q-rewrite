...
def main() -> int:
    ...
    The model is determined from the "MODEL" environment variable defined in the `.env` file, if it exists,
    ...
    if os.environ.get("MODEL") is None:
        logger.error('env var "MODEL" is not set')

        return 1

    compose_file_path = PROJECT_ROOT_PATH / "deployments" / "compose.development.yml"

    logger.info(f'starting model "{os.environ["MODEL"]}" in docker')
